...
export function ChatPanel({ title, messages, typingUsers, currentUser, targetUser, onReply, onReplyPrivately, isDark, onToggleTheme, onMenuClick }) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center my-2 sm:my-4 opacity-80 animate-fade-in">
          <div className="bg-yellow-500/10 text-yellow-600 dark:text-yellow-500 text-xs px-3 sm:px-4 py-2 rounded-2xl flex items-center gap-2 text-center max-w-sm sm:max-w-md border border-yellow-500/20">
            <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
            <p className="text-left">Messages are end-to-end encrypted with AES-128. No one outside of this chat, not even the server, can read them.</p>
          </div>
        </div>
