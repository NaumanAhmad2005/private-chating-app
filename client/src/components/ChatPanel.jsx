...
export function ChatPanel({ title, messages, typingUsers, currentUser, targetUser, onReply, onReplyPrivately, isDark, onToggleTheme, onMenuClick }) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center my-2 sm:my-4 opacity-100 animate-fade-in">
          <button
            onClick={() => setShowE2EEDetails(!showE2EEDetails)}
            onMouseEnter={() => setShowE2EEDetails(true)}
            onMouseLeave={() => setShowE2EEDetails(false)}
            className="bg-chat-surface hover:bg-chat-surface/80 text-xs px-3 sm:px-4 py-2 rounded-2xl flex items-center gap-2 text-center max-w-sm sm:max-w-md border border-chat-border transition-all cursor-pointer"
            style={{ color: 'var(--chat-text)' }}
          >
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" />
            </svg>
            <p className="text-left select-none transition-all duration-200" style={{ color: 'var(--chat-text)' }}>
              {showE2EEDetails 
                ...
                <React.Fragment key={message.id}>
                  {showDateSeparator && (
                    <div className="flex items-center justify-center my-2 sm:my-3">
                      <span className="text-xs px-3 py-1 rounded-full bg-chat-surface border border-chat-border select-none" style={{ color: 'var(--chat-text-muted)' }}>
